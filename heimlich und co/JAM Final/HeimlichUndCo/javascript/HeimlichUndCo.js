...
var arrFields = [ 0, 0, 0, 0, 0, 0, 0, 0, 7, 0, 0, 0, 0, 0, 0, 0, 0 ];
...
addListener('standardEvent', function(event) {
	...
	if (arr.length == 17) {
		...
		if (str == "HOST") {
			...
			//document.getElementById("wuerfel").setAttribute("visibility","hidden");
...
addListener('STARTNOW', function(event) {
	var stringFromServer = event.data;
	var arr = stringFromServer.split(',');
	Spieleranzahl = arr[1];
	Agentenanzahl = arr[0];
...
addListener('PLAYERAMOUNT', function(event) {
	var stringFromServer = event.data;
	
	
	sendDataToServer("PLAYERAMOUNT," + anzahlPlayer);
	playerMessage = stringFromServer;
	document.getElementById("status").innerHTML = playerMessage;
});
...
function rollDice() {
	...
	document.getElementById("rollimg").src = "/HeimlichUndCo/images/wurfel_"
			+ d1 + ".png"
	var diceTotal = d1;
	zuege = d1;
...
function startscreen() {
	console.log("startaufruf");
	
	 anzahlPlayer = parseInt(document.getElementById("countPlayer").value);
	 anzahlKi = parseInt(document.getElementById("countKi").value);
